...
def BruteForcePotential2(x_target,x_source, m,h=None,G=1.):
    if h==None: h = np.zeros(x_target.shape[0])
...
def ComputeClouds(filepath , options):
    ...
    if ".hdf5" in filepath: # we have a lone snapshot, no snapdir
        ...
        print(filepath, snapnum)
        dummy1 = glob(filepath+"/*.hdf5")
...
#    print(snapdir, snapnum)
    ...
    print ('Checking existence..', snapdir, snapname, snapnum)
...
#    print("Total SFR: %g"%particle_data["StarFormationRate"].sum())
    ...
    x += np.random.normal(size=x.shape)*1e-6
...
#        print(len(c))
        ...
        if len(bound_groups)>1000:
            cluster_id = "Cloud"+ ("%d"%i).zfill(int(np.log10(len(bound_groups))+1))
        else:
            cluster_id = "Cloud0"+ ("%d"%i).zfill(int(np.log10(len(bound_groups))+1))
